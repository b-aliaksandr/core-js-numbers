...
function roundToPowerOfTen(num, pow) {
  if (pow === 0) return num;
  const secondPartOfNum = num % 10 ** (pow + 1);
  const halfNumberOfPowerOfTen = 10 ** (pow + 1) / 2;
  const isLargest = secondPartOfNum >= halfNumberOfPowerOfTen;
  const firstPartOfNum = Number.parseInt(num / 10 ** pow, 10) * 10 ** pow;
  return isLargest ? firstPartOfNum + 10 ** pow : firstPartOfNum;
}
...
function toNumber(value, def) {
  return Number.isFinite(Number.parseInt(value, 10)) ? value : def;
}
...
function getSumOfNumbers(x1, x2, x3) {
  const sum = [x1, x2, x3].reduce((acc, cur) => acc + cur, 0);
